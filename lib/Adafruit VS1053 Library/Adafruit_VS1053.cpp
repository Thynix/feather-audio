...
boolean Adafruit_VS1053_FilePlayer::playFullFile(const char *trackname, SdFs *sd) {
  if (!startPlayingFile(trackname, sd))
    return false;
...
boolean Adafruit_VS1053_FilePlayer::startPlayingFile(const char *trackname, SdFs *sd) {
  // reset playback
  sciWrite(VS1053_REG_MODE, VS1053_MODE_SM_LINE1 | VS1053_MODE_SM_SDINEW |
